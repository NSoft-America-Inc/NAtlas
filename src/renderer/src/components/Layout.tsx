  ...
  return (
    <div className="w-screen h-screen bg-background text-foreground overflow-hidden">
      <ResizablePanelGroup orientation="horizontal" className="h-full w-full">
          ...
          <div className="px-5 py-4 border-t border-border/40 bg-muted/10">
            <div className="flex flex-col gap-0.5 truncate">
              ...
              <span className="text-[9px] text-muted-foreground/60 truncate">
                Phase 2{' '}
                {updateInfo?.current_version ? `v${updateInfo.current_version}` : 'v1.0.0-beta.1'}
              </span>
            </div>
          </div>
        </ResizablePanel>
        ...
        <ResizablePanel
          defaultSize="84%"
          className="flex flex-col h-full bg-background/95 overflow-hidden"
        >
            ...
            <div className="flex items-center justify-between px-5 py-3.5 bg-indigo-950/45 border-b border-indigo-500/20 text-indigo-200 select-none animate-in slide-in-from-top duration-300 shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs font-medium truncate">
                  NAtlas의 새로운 버전(
                  <span className="text-indigo-300 font-semibold">{updateInfo.latest_version}</span>
                  )이 준비되었습니다. (현재 버전: {updateInfo.current_version})
                </span>
              </div>
