  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.ok) {
        ...
        setGlobalSettings({ llmwiki_root: llmwikiRoot })
    ...
    <div className="flex-1 flex flex-col h-full bg-background select-text">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 max-w-2xl">
        <div className="border border-border rounded-xl bg-card/10 p-6 space-y-6 shadow-sm">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                className="flex-1 bg-muted/20 border-border focus-visible:ring-indigo-500/50 focus-visible:border-indigo-500/50 font-mono text-xs text-slate-200"
              />
              <Button
                variant="outline"
                onClick={handleOpenFolder}
                disabled={isLoading || saveMutation.isPending}
                className="bg-muted/40 hover:bg-muted text-foreground border-border hover:border-muted-foreground/30 transition-all duration-300 select-none"
              >
          ...
          {/* Action buttons */}
          <div className="flex justify-end select-none pt-4 border-t border-border/40">
            <Button
              onClick={handleSave}
              disabled={isLoading || saveMutation.isPending || !llmwikiRoot.trim()}
